...
class KubeLeaseRepositorySuite extends CatsEffectSuite with K3dClientProvider {
      ...
      KubeLeaseRepository[IO](testLabel).use { repository =>
        for {
          ...
          _ <- repository.watcher
            ...
            .drain
            .start
          (lease, release) <- repository.acquire(LeaseID("introspection-test-lease").get, holderID).allocated
  ...
  test("handle returned by get allows introspecting lease properties") {
    withKubeClient { implicit client =>
      KubeLeaseRepository[IO](testLabel).use { repository =>
        for {
          (lease, release) <- repository.acquire(LeaseID("get-test-lease").get, holderID).allocated
  ...
  test("reacquired stray lease is considered the same as acquired") {
    withKubeClient { implicit client =>
      ...
      for {
        _ <- client.leases.namespace(namespace).delete(leaseID)
        simulatedStrayLeaseStatus <- client.leases
          .namespace(namespace)
          .createOrUpdate(k8sLease(leaseID, holderID, List(testLabel), Instant.now, parameters.leaseDuration))
