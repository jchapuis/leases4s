...
trait LeaseRepository[F[_]] {
  ...
  /** Returns a resource representing acquisition of the lease and release upon finalization.
    ...
    */
  def acquire(id: LeaseID, holderID: HolderID)(implicit
...
object LeaseRepository {
  ...
  /** Parameters for lease acquisition and renewal
    ...
    * @param baseOnErrorRetryDelay base delay between retries in case of error, grows exponentially with each retry
    */
  final case class LeaseParameters(
      ...
      baseOnErrorRetryDelay: FiniteDuration = DefaultOnErrorRetryDelay
  )
