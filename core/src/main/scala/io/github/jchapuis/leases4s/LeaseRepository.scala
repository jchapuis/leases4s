...
/** Defines a distributed lease repository, with the ability to acquire leases with automatic renewal, check for other
  * existing leases and watch for related events.
  */
trait LeaseRepository[F[_]] {
  ...
  /** Returns a resource representing acquisition of the lease and release upon finalization. The resource acquire
    * action is semantically blocking until the lease is acquired. Once acquired, the lease is automatically renewed by
    * a fiber supervised by the resource, according to passed parameters.
    * @param id
    *   lease ID
    * @param holderID
    *   holder ID
...
object LeaseRepository {
  ...
  /** Parameters for lease acquisition and renewal
    * @param leaseDuration
    *   duration of the lease, after which the lease can be considered expired (unless it is renewed, which happens
    *   automatically while the lease is actively held)
    * @param renewalFrequencyRatio
    *   ratio of the lease duration at which the lease is renewed: this should be smaller than 1 to ensure safe
    *   operation (typically 1/3)
    * @param initialOnErrorRetryDelay
  ...
  /** Creates a new lease repository
    ...
    */
  def kubernetes[F[_]: Async: Logger](label: Label, labels: Label*)(implicit
      client: KubernetesClient[F],
      namespace: Namespace = Namespace.Default,
      parameters: LeaseParameters = LeaseParameters.Default
  ): Resource[F, LeaseRepository[F]] = KubeLeaseRepository[F](label, labels*)
}
