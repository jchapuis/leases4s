...
import besom.api.kubernetes.rbac.v1.inputs.{
  PolicyRuleArgs,
  RoleRefArgs,
  SubjectArgs
}
import besom.api.kubernetes.rbac.v1.{
  Role,
  RoleArgs,
  RoleBinding,
  RoleBindingArgs
...
def leasesAccessServiceAccount(
    namespace: String
    ...
    Some(
      List(
        PolicyRuleArgs(
          ...
          verbs =
            List("get", "list", "watch", "create", "update", "patch", "delete")
        )
      )
    )
  )
...
private def roleBinding(
    roleName: String,
    serviceAccountName: String,
    namespace: String
)(using Context) = RoleBinding(
  leaseAccess,
  RoleBindingArgs(
    roleRef = RoleRefArgs(
